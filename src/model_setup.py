...
def build_retfound_regression(weights_path, device, *, trusted_checkpoint=True, img_size=384):
    """
    ...
    model = RETFound_mae(num_classes=1, global_pool="avg", img_size=img_size)
    ...
    # Resize positional embeddings if input resolution differs from pretrain.
    if 'pos_embed' in checkpoint_model and checkpoint_model['pos_embed'].shape != state_dict['pos_embed'].shape:
        checkpoint_model['pos_embed'] = timm.models.vision_transformer.resize_pos_embed(
